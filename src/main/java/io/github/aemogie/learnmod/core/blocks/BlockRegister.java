...
public class BlockRegister {
	...
	public static Block register(ResourceLocation name, Block block) {
		return register(name, block, References.MISC_GROUP);
	}
	...
	public static Block register(ResourceLocation name, Block block, ItemGroup group) {
		return register(name, block, group, "opaque");
	}
	...
	public static <T extends Block> T register(ResourceLocation name, T block, ItemGroup group, String render_mode) {
		return register(name, block, render_mode, new BlockItem(block, new Item.Properties().tab(group)));
	}
	...
	public static <T extends Block> T register(ResourceLocation name, T block, String render_mode, BlockItem blockItem) {
		...
		switch (render_mode) {
			...
			case "opaque":
				...
				break;
			default:
				LOGGER.error("Invalid render mode for block: " + name + " : " + render_mode);
				System.exit(-1);
		}
...
//					modifyString("Successfully added opaque block: "),
//					modifyString(Objects.requireNonNull(block.getRegistryName()).toString(), GREEN))
//			);
		});
		BlockLists.TRANSLUCENT_BLOCKS.forEach(block -> {
			event.getRegistry().register(block);
//			LOGGER.info(new LoggerFormat(
//					modifyString("Successfully added translucent block: "),
//					modifyString(Objects.requireNonNull(block.getRegistryName()).toString(),GREEN, BOLD))
//			);
		});
	}
	...
	public static void registerBlockItems(final RegistryEvent.Register<Item> event) {
		event.getRegistry().registerAll(BlockLists.BLOCK_ITEMS.toArray(new Item[0]));
